...
function search()
{
	...
	<pp:var name="tempUrl" value="'action=group&method=frameListGroup&mode='.$mode"/>
	document.all.listInfo.src = "index.php[@encrypt_url($tempUrl)]&sqlCon="+sqlConStr;
	//document.all.listInfo.src = "index.php?action=group&method=frameListGroup&mode=[$mode]&sqlCon="+sqlConStr;
}
</script>
</head>
<body>
  ...
  <pp:var name="tempUrl" value="'action=group&method=frameListGroup&appName=' .$appName .'&isText=' .$isText .'&mode='.$mode"/>
...
<!--<script language="javascript" type="">
  ...
  fieldList[1]=sqlcondition.newFieldbean("groupNo","组编码","varchar","","");
  fieldList[2]=sqlcondition.newFieldbean("groupName","组名称","varchar","","");
