...
<div class="ctrl_bar">
		<div class="title fl">
			官方活动信息<?php if($item[0]["event_block"]==1):?> <span class="pink_link">(已阻止发布)</span><?php endif;?>
		</div>
		<ul class="fr ctrl_link">
			<li id="ctrl_3"><a onClick="javascript:return confirm('<?php if($item[0]["event_block"]==0):?>是否阻止此活动的发布？<?php else:?>是否恢复此条发言的发布?<?php endif;?>')" href="index.php<?php echo runFunc('encrypt_url',array('action=cms&method=event_block&id='.$this->_tpl_vars["IN"]["id"].'&type=share'));?>"><?php if($item[0]["event_block"]==0):?>停止发布<?php else:?>恢复发布<?php endif;?></a></li>
			<li id="ctrl_2"><a href="index.php<?php echo runFunc('encrypt_url',array('action=cms&method=eventList&type=share'));?>">退出</a></li>
		</ul>
	</div>
		 ...
		 <fieldset class="admin_fieldset">
			...
			<table class="admin_edit_table">
				...
				<tr>
					<th>外链：</th>
					<td>
					<?php if($item[0]["out_link"]!=""):?>
					<a target="_blank" class="pink_link_s" href="<?php echo $item[0]["out_link"];?>">查看</a>
					<?php else:?>
					无
					<?php endif;?>
					</td>
					...
					<div class="item_comments_box fl ">	
						...
						<div class="item_comment_box oh" id="comment_<?php echo $comment["id"]?>">
							...
							<?php if($comment["block"]==0):?>
								<a onClick="javascript: return confirm('是否确认阻止此条评论的发布,并发送邮件警告这个会员？')" class="pink_link_s" href="<?php echo runFunc('encrypt_url',array('action=cms&method=event_comment_block&id='.$comment["id"]."&event_id=".$this->_tpl_vars["IN"]["id"]."&comment_id=".$comment["id"]));?>">阻止发布</a>			
							<?php else:?>
								<a onClick="javascript: return confirm('是否确认恢复此条评论的发布？')" class="pink_link_s" href="<?php echo runFunc('encrypt_url',array('action=cms&method=event_comment_block&id='.$comment["id"]."&event_id=".$this->_tpl_vars["IN"]["id"]."&comment_id=".$comment["id"]));?>">恢复发布</a>
							<?php endif;?>
							</div>
						</div>
						<?php endforeach;?>
