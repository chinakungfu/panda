...
$items = runFunc("getAdminHotBrandCategories",array($this->_tpl_vars["IN"]["page"],15));
	
$count = runFunc("getAdminHotBrandCategories",array($this->_tpl_vars["IN"]["page"],15,true));
?>
...
<div class="cms_right fr">
	...
	<table class="order_list <?php if(($i++%2)==""){echo "pink";}?>">
		<tr>
			<td width="40%"><a href="<?php echo runFunc('encrypt_url',array('action=cms&method=hot_brand_category_edit&id='.$item["id"].'&type=products'));?>"><?php echo $item["name"];?></a></td>
			<td width="25%"><?php echo $_GLOBAL['published_'.$item["published"]];?></td>
			<td width="25%"><?php echo $item["created"];?></td>
			<td width="10%"><a href="<?php echo runFunc('encrypt_url',array('action=cms&method=hot_brand_category_edit&id='.$item["id"].'&type=products'));?>">编辑</a> | <a onClick="javascript:return confirm('是否确认删除该分类')" href="<?php echo runFunc('encrypt_url',array('action=cms&method=hot_brand_category_delete&id='.$item["id"].'&type=products'));?>">删除</a></td>
		</tr>
	</table>
	<?php endforeach;
	else:
	...
	<?php echo runFunc("adminPageNavi",array($count[0]["count"],15,"cms","hot_brand_category_list","products",$page,$this->_tpl_vars["IN"]["order_status"],$this->_tpl_vars["IN"]["search_word"],$this->_tpl_vars["IN"]["search_type"]));?>
</div>
</div>
<?php
$inc_tpl_file=includeFunc(<<<LNMV
