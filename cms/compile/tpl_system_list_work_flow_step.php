...
function adddata()
{
	...
	document.forms[0].method.value="addWorkFlowStep";
	document.forms[0].submit();
}
function editdata()
{
	document.forms[0].action.value="cms";
	document.forms[0].method.value="editWorkFlowStep";
	if (checkdata())
	{
		document.forms[0].submit();
	}
...
{
	document.forms[0].action.value="cms";
	document.forms[0].method.value="selfWorkFlowStep";
	if (checkdata())
	{
		document.forms[0].submit();
	}
...
{
	document.forms[0].action.value="cms";
	document.forms[0].method.value="delWorkFlowStep";
	if (checkdata())
	{
		document.forms[0].submit();
	}
...
function checkdata()
{
	...
	for(i=0;i<chks.length;i++){
		if(chks[i].checked){
			document.forms[0].flowStepId.value=chks[i].value;
			bool=1;
			break;
		}
	}
...
<div class="main_content">
   	<div class="main_content_nav">当前位置：系统管理>>系统管理>>工作流步骤</div>
...
<div class="search_content detailMember">
 ...
 <form action="index.php" method="POST">  
  ...
  <input type="hidden" name="flowStepId">
    ...
    <?php $this->_tpl_vars["result"]=runFunc('listWorkFlowStep',array()); ?>
    <table class="tableList" border="1" id="list" width="100%">    
      <tr>
    		<td class="listHeader">选中</td>
          	<td class="listHeader">工作流步骤名称</td>
          	<td class="listHeader">工作流步骤介绍</td>
       </tr>  
       
       <?php if(!empty($this->_tpl_vars["result"]["data"])){ 
 foreach ($this->_tpl_vars["result"]["data"] as $this->_tpl_vars['key']=>$this->_tpl_vars['var']){ ?>
       <tr>
          <td class="tdListItem"><input type="checkbox" name="checks" value="<?php echo $this->_tpl_vars["var"]["flowStepId"];?>" onclick="selectConCheck();"></td>
          <td class="tdListItem"><?php echo $this->_tpl_vars["var"]["flowStepName"];?></td>
          <td class="tdListItem"><?php echo $this->_tpl_vars["var"]["flowStepRemark"];?></td>
       </tr>
       <?php  }
} ?>
    </table>
