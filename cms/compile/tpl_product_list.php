...
$items = runFunc("getGoodsList",array("inside",$page,20,false,$this->_tpl_vars["IN"]["sort"],$this->_tpl_vars["IN"]["key"],$this->_tpl_vars["IN"]["status"],$this->_tpl_vars["IN"]["search_word"],$this->_tpl_vars["IN"]["product_category"],$this->_tpl_vars["IN"]["brand_id"]));
$items_count = runFunc("getGoodsList",array("inside",1,20,true,$this->_tpl_vars["IN"]["sort"],$this->_tpl_vars["IN"]["key"],$this->_tpl_vars["IN"]["status"],$this->_tpl_vars["IN"]["search_word"],$this->_tpl_vars["IN"]["product_category"],$this->_tpl_vars["IN"]["brand_id"]));
...
<div class="search_bar">
			...
			<a style="margin-left:10px;" class="fl" href="<?php echo runFunc('encrypt_url',array('action=cms&method=product_list&type=products'));?>">清空搜索条件</a>
		...
		<ul>
			<li><a <?php if($this->_tpl_vars["IN"]["status"] == ""){echo "class='active_filter'";}?> href="<?php echo runFunc('encrypt_url',array('action=cms&method=product_list&type=products&sort='.$this->_tpl_vars["IN"]["sort"].'&key='.$this->_tpl_vars["IN"]["key"].'&search_word='.$this->_tpl_vars["IN"]["search_word"].'&product_category='.$this->_tpl_vars["IN"]["product_category"]."&brand_id=".$this->_tpl_vars["IN"]["brand_id"]));?>">全部</a></li>
			<li>|</li>
			<li><a <?php if($this->_tpl_vars["IN"]["status"] == "1"){echo "class='active_filter'";}?> href="<?php echo runFunc('encrypt_url',array('action=cms&method=product_list&type=products&status=1&sort='.$this->_tpl_vars["IN"]["sort"].'&key='.$this->_tpl_vars["IN"]["key"].'&search_word='.$this->_tpl_vars["IN"]["search_word"].'&product_category='.$this->_tpl_vars["IN"]["product_category"]."&brand_id=".$this->_tpl_vars["IN"]["brand_id"]));?>">发布</a></li>
			<li>|</li>
			<li><a <?php if($this->_tpl_vars["IN"]["status"] == "2"){echo "class='active_filter'";}?> href="<?php echo runFunc('encrypt_url',array('action=cms&method=product_list&type=products&status=2&sort='.$this->_tpl_vars["IN"]["sort"].'&key='.$this->_tpl_vars["IN"]["key"].'&search_word='.$this->_tpl_vars["IN"]["search_word"].'&product_category='.$this->_tpl_vars["IN"]["product_category"]."&brand_id=".$this->_tpl_vars["IN"]["brand_id"]));?>">不发布</a></li>
		</ul>
	</div>
...
<table class="order_list <?php if(($i++%2)==""){echo "pink";}?>">
	<tbody>
		<tr>
			...
			<td width="32%" style="text-align:left;padding:0 10px;" width="20%"><a href="<?php echo runFunc('encrypt_url',array('action=cms&method=product_edit&type=products&id='.$item["goodsid"]));?>"><?php echo $item["goodsTitleCN"];?></a></td>
			...
			<td width="14%"><a href="<?php echo runFunc('encrypt_url',array('action=cms&method=product_edit&type=products&id='.$item["goodsid"].'&page='.$page));?>">编辑</a> <a onClick="javascript: return confirm('是否确认删除这个此项？')" href="<?php echo runFunc('encrypt_url',array('action=cms&method=product_delete&delete_type=goods&id='.$item["goodsid"].'&page='.$page));?>">删除</a> <a href="<?php echo runFunc('encrypt_url',array('action=cms&method=product_update&type=products&dataType=one&id='.$item["goodsid"].'&page='.$page));?>">更新</a>
			<a href="<?php echo runFunc('encrypt_url',array('action=cms&method=product_update_surprise&type=products&dataType=one&backType=inside&id='.$item["goodsid"].'&page='.$page));?>">抓取</a>
			</td>
		</tr>
	</tbody>
</table>
...
<?php echo runFunc("adminPageNavi",array($items_count[0]["count"],20,"cms","product_list",'products&status='.$this->_tpl_vars["IN"]["coupon_status"].'&sort='.$this->_tpl_vars["IN"]["sort"].'&key='.$this->_tpl_vars["IN"]["key"].'&search_word='.$this->_tpl_vars["IN"]["search_word"].'&product_category='.$this->_tpl_vars["IN"]["product_category"]."&brand_id=".$this->_tpl_vars["IN"]["brand_id"],$page));?>
</div>
</div>
<?php
$inc_tpl_file=includeFunc(<<<LNMV
