...
<div class="ctrl_bar">
	...
	<div class="search_bar">
		<form action="index.php" method="post">
			...
			<input type="hidden" name="method" value="sells_total"/>
	...
	<div class="gray_line_box">
		 <fieldset class="admin_fieldset">
		    <legend><?php echo $year;?>年<?php echo $month;?>月<?php if($total_type==1){echo "销量";}else{echo "金额";}?>统计</legend>
	<div id="place_legend1" class="flace_legend"></div>
		 ...
		 <fieldset class="admin_fieldset">
		    <legend><?php echo $year;?>年度<?php if($total_type==1){echo "销量";}else{echo "金额";}?>统计</legend>
...
<?php for ($d=1;$d<=$month_days;$d++){
	...
	$count = runFunc("getSellsTotal",array($year,$month,$d,"inside"));
	$o_count = runFunc("getSellsTotal",array($year,$month,$d,"outside"));
	if($count[0]['count'] == ""){
	...
	if($o_count[0]['count'] == ""){
		...
		$o_count = $o_count[0]['count'];
	}
	
	$total_array[$d] = "[".$d.", ".$count."]";
	$o_total_array[$d] = "[".$d.", ".$o_count."]";
...
<?php for ($m=1;$m<=12;$m++){
	...
	$m_count = runFunc("getSellsTotal",array($year,$m,"","inside"));
	$o_m_count = runFunc("getSellsTotal",array($year,$m,"","outside"));
	...
	if($o_m_count[0]['count'] == ""){
	
		$o_m_count = 0;
	}else{
		$o_m_count = $o_m_count[0]['count'];
	}
	
	
	$m_total_array[$m] = "[".$m.", ".$count."]";
	
	$o_m_total_array[$m] = "[".$m.", ".$o_m_count."]";
...
<script type="text/javascript">
	$(function(){
		 ...
		 var o_d2 =  [<?php echo implode(",", $o_total_array)?>];

		 
		 var d3 = [<?php echo implode(",", $m_total_array)?>];
		 var o_d3 = [<?php echo implode(",", $o_m_total_array)?>];

		   $.plot(
				    $("#placeholder"), [
		                   		    { label: "<?php echo $month;?>月份站内产品<?php if($total_type==1){echo "销量";}else{echo "金额";}?>",  data: d2},
		                   		   { label: "<?php echo $month;?>月份站外搜索产品<?php if($total_type==1){echo "销量";}else{echo "金额";}?>",  data: o_d2}
		   ...
		   $("#placeholder").bind("plothover", function (event, pos, item) {
		        ...
		        $("#y").text(pos.y.toFixed(2));
		            ...
		            if (item) {
		                if (previousPoint != item.dataIndex) {
		                    ...
		                    showTooltip(item.pageX+5, item.pageY,"<?php echo $month;?>月"+ item.datapoint[0] + "日<?php if($total_type==1){echo "销量";}else{echo "金额";}?>:"+y);
		                }
		            }
		            else {
		                $("#tooltip").remove();
		    ...
		    $.plot(
				    $("#placeholder-2"), [
		                   		    { label: "<?php echo $year;?>年度<?php if($total_type==1){echo "站内产品销量";}else{echo "金额";}?>",  data: d3},
		                   		    { label: "<?php echo $year;?>年度<?php if($total_type==1){echo "站外搜索产品销量";}else{echo "金额";}?>",  data: o_d3}
		        ...
		        $("#y").text(pos.y.toFixed(2));
		            ...
		            if (item) {
		                if (previousPoint != item.dataIndex) {
		                    ...
		                    showTooltip(item.pageX+5, item.pageY,"<?php echo $year;?>年"+ x +"月<?php if($total_type==1){echo "销量";}else{echo "金额";}?>:"+y);
		                }
		            }
		            else {
		                $("#tooltip").remove();
