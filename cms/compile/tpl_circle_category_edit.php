...
$item = runFunc("getCircleCategory",array($this->_tpl_vars["IN"]["id"]));
?>
...
<div class="ctrl_bar">
		...
		<ul class="fr ctrl_link">
			...
			<li id="ctrl_2"><a href="index.php<?php echo runFunc('encrypt_url',array('action=cms&method=circle_category&type=share'));?>">取消</a></li>
		</ul>
	</div>
	<div class="gray_line_box">
		...
		<input type="hidden" name="method" value="circle_category_save" />
		<input type="hidden" name="id" value="<?php echo $this->_tpl_vars["IN"]["id"];?>" />
			<table class="admin_edit_table">
				<tr>
					<th>分类名称</th>
					<td><input type="text" name="title" id="title" class="dark_border input_bar_long required" value="<?php echo $item[0]["title"]?>"/></td>
				</tr>
				<tr>
					<th>发布</th>
					<td>
