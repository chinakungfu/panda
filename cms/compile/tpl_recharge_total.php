...
<div class="ctrl_bar">
	...
	<div class="search_bar">
		<form action="index.php" method="post">
			...
			<select id="total_type" class="select_filter fr" name="total_type">
				...
				<option <?php if($total_type == 1){echo "selected=selected";}?> value="1">充值次数</option>
				<option <?php if($total_type == 2){echo "selected=selected";}?> value="2">充值金额</option>
			</select>
			<label for="search_word" class="fr">统计年月：</label>
			<input type="hidden" name="action" value="cms"/>
			<input type="hidden" name="method" value="recharge_total"/>
	...
	<div class="gray_line_box">
		 <fieldset class="admin_fieldset">
		    <legend><?php echo $year;?>年<?php echo $month;?>月充值<?php if($total_type==1){echo "数量";}else{echo "金额";}?>统计</legend>
	<div id="place_legend1" class="flace_legend"></div>
		 ...
		 <fieldset class="admin_fieldset">
		    <legend><?php echo $year;?>年度充值<?php if($total_type==1){echo "数量";}else{echo "金额";}?>统计</legend>
...
<?php for ($d=1;$d<=$month_days;$d++){
	...
	$count = runFunc("getRechargePayTotal",array($total_type,$year,$month,$d));
	if($count[0]['count'] == ""){
...
<?php for ($m=1;$m<=12;$m++){
	...
	$m_count = runFunc("getRechargePayTotal",array($total_type,$year,$m));
	if($m_count[0]['count'] == ""){
...
<script type="text/javascript">
	$(function(){
		 ...
		 var d3 = [<?php echo implode(",", $m_total_array)?>];
		   ...
		   $.plot(
				    $("#placeholder"), [
		                   		    { label: "<?php echo $month;?>月份充值<?php if($total_type==1){echo "数量";}else{echo "金额";}?>",  data: d2}],
		   ...
		   $("#placeholder").bind("plothover", function (event, pos, item) {
		        ...
		        $("#y").text(pos.y.toFixed(2));
		            ...
		            if (item) {
		                if (previousPoint != item.dataIndex) {
		                    ...
		                    showTooltip(item.pageX+5, item.pageY,"<?php echo $month;?>月"+ item.datapoint[0] + "日充值<?php if($total_type==1){echo "数量";}else{echo "金额";}?>:"+y);
		                }
		            }
		            else {
		                $("#tooltip").remove();
		    ...
		    $.plot(
				    $("#placeholder-2"), [
		                   		    { label: "<?php echo $year;?>年度充值<?php if($total_type==1){echo "数量";}else{echo "金额";}?>",  data: d3}],
		        ...
		        $("#y").text(pos.y.toFixed(2));
		            ...
		            if (item) {
		                if (previousPoint != item.dataIndex) {
		                    ...
		                    showTooltip(item.pageX+5, item.pageY,"<?php echo $year;?>年"+ x +"月充值<?php if($total_type==1){echo "数量";}else{echo "金额";}?>:"+y);
		                }
		            }
		            else {
		                $("#tooltip").remove();
