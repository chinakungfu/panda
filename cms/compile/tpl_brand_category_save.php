...
if($id ==""){
	...
	$sql = '';
		...
		$sql = "update cms_product_brand_category set $sql where id = '{$id}'";
...
runFunc("makeAdminLog",array("新增商品品牌分类 ".$this->_tpl_vars["IN"]["name"],$this->_tpl_vars["name"]));
}else{
$this->_tpl_vars["name"]=runFunc('readSession',array());
runFunc("makeAdminLog",array("更新商品品牌分类 ".$this->_tpl_vars["IN"]["name"],$this->_tpl_vars["name"]));
}
