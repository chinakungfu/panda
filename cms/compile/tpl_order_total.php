...
<div class="ctrl_bar">
	...
	<div class="search_bar">
		<form action="index.php" method="post">
			...
			<select id="total_type" class="select_filter fr" name="total_type">
				<option value="">统计类型</option>
				<option <?php if($total_type == 1){echo "selected=selected";}?> value="1">订单数</option>
				<option <?php if($total_type == 2){echo "selected=selected";}?> value="2">订单金额</option>
			</select>
			<label for="search_word" class="fr">统计年月：</label>
			<input type="hidden" name="action" value="cms"/>
			<input type="hidden" name="method" value="order_total"/>
	...
	<div class="gray_line_box">
		 <fieldset class="admin_fieldset">
		    <legend><?php echo $year;?>年<?php echo $month;?>月成交<?php if($total_type==1){echo "订单数量";}else{echo "额";}?>统计</legend>
		 ...
		 <fieldset class="admin_fieldset">
		    <legend><?php echo $year;?>年度成交<?php if($total_type==1){echo "订单数量";}else{echo "额";}?>统计</legend>
...
for ($d=1;$d<=$month_days;$d++){
	...
	$count = runFunc("getOrderSuccessPayTotal",array($total_type,$year,$month,$d));
	if($count[0]['count'] == ""){
	...
	$main_count_m = $main_count_m + $count;
...
for ($m=1;$m<=12;$m++){
	...
	$m_count = runFunc("getOrderSuccessPayTotal",array($total_type,$year,$m));
	if($m_count[0]['count'] == ""){
	...
	$(function(){
		 ...
		 var d3 = [<?php echo implode(",", $m_total_array)?>];
		   ...
		   $.plot(
				    ...
				    $("#placeholder"), [
		                   		    { label: "<?php echo $month;?>月份成交<?php if($total_type==1){echo "订单数量";}else{echo "额";}?> 总计：<?php echo $main_count_m?>",  data: d2}],
		   ...
		   $("#placeholder").bind("plothover", function (event, pos, item) {
		        ...
		        $("#y").text(pos.y.toFixed(2));
		            ...
		            if (item) {
		                if (previousPoint != item.dataIndex) {
		                    ...
		                    showTooltip(item.pageX+5, item.pageY,"<?php echo $month;?>月"+ item.datapoint[0] + "日成交<?php if($total_type==1){echo "订单数量";}else{echo "额";}?>:"+y);
		                }
		            }
		            else {
		                $("#tooltip").remove();
		    ...
		    $.plot(
				    $("#placeholder-2"), [
		                   		    { label: "<?php echo $year;?>年度成交订单<?php if($total_type==1){echo "数量";}else{echo "金额";}?> 总计：<?php echo $main_count;?>",  data: d3}],
		        ...
		        $("#y").text(pos.y.toFixed(2));
		            ...
		            if (item) {
		                if (previousPoint != item.dataIndex) {
		                    ...
		                    showTooltip(item.pageX+5, item.pageY,"<?php echo $year;?>年"+ x +"月成交<?php if($total_type==1){echo "订单数量";}else{echo "额";}?>:"+y);
		                }
		            }
		            else {
		                $("#tooltip").remove();
