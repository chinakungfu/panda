...
function MzTreeView(Tname)
{
  ...
  {
    "highLight" : "#0A246A",
    "highLightText" : "#FFFFFF",//鼠标点击后结点文字颜色
    "mouseOverBgColor" : "#D4D0C8"//鼠标点击后颜色
  };
  this.icons    = {
    L0        : 'L0.gif',  //┏
    L1        : 'L1.gif',  //┣
...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  if( node.ctrl.length > 0 ) HTMCHK = "<input type='checkbox' name='"+ node.ctrl +"' value='"+ sid +
  	"' id='"+ this.name +"_checbox_"+ id +"' style='clip:rect(1 1 1 1);'"+
  ...
  HTML += "<IMG "+
    ...
    "oncontextmenu=\"return "+ this.name +".nodeRightClick('"+ id +"')\" "+   //这里是修改的部分，添加了右键触发事件20090320zxqer
...
//节点链接右键单击事件处理方法   
//id 客户端树节点的 id   
MzTreeView.prototype.nodeRightClick = function(id)   
{  
  var source = this.N[this.node[id].sourceIndex]; 
  //这里的rightmethod是要传进来的自定义函数
  return eval(this.getAttribute(source, "rightmethod"));   
};
