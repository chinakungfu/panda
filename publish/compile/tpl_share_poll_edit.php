...
$poll_items = runFunc("getPollItems",array($item["id"]));
	...
	<form id="list_submit_form" action="/publish/index.php" method="post" onSubmit="">
		...
		<div class="narrow_box">
			<div class="narrow_left fl">
				...
				<div class="poll_item_contain gray_line_box narrow_line_box">
						<table class="list_editor_table">
							<tr>
								<th>Notice:</th>
								<td style="color:#FB6666">If you update your poll, this poll's votes will return to 0.</td>
							</tr>
							<tr>
								<th style="line-height: 46px;vertical-align: top;">Poll Name</th>
								<td>
									<input class="list_editor_input big_input required" type="text" id="poll_name" maxlength="255" value="<?php echo $item["name"];?>" name="poll_name"/>
									<br/>
							...
							<tr>
								...
								<td>
									<input class="list_editor_input big_input datePicker required" style="width: 104px;" type="text" value="<?php echo $item["end_time"];?>" name="close_date"/>
									<br/>
						...
						<div class="poll_item_main_box oh">
							...
							<div class="poll_item_box fl">
								<div class="remove_poll_item_box">
									<a class="remove_poll_item hide" style="display: inline;">Remove</a>
								</div>
								<div class="poll_img_content">
									
									<div class="poll_img_box"><img src="<?php echo $poll_item["goodsImgURL"];?>_310x310.jpg" class="fl"></div>
									<textarea class="poll_diy_title" name="poll_diy_title[]"><?php if(trim($poll_item["title"])!=""){$poll_item["goodsTitleCN"] =  $poll_item["title"]; }?><?php echo $poll_item["goodsTitleCN"]?></textarea>
									<div class="poll_item_detail"> ￥<?php echo number_format($poll_item["goodsUnitPrice"], 2, '.', ',');?>
										<input type="hidden" name="poll_items[]" value="<?php echo $poll_item["goodsid"];?>">
									</div>
								</div>
							</div>
							<?php endforeach;?>
							<?php if(count($poll_items)<3):?>
							<div class="poll_item_box fl">
								<div class="remove_poll_item_box">
						...
						<input type="hidden" name="poll_item_num" id="poll_item_num" value="<?php echo count($poll_items);?>"/>
				</div>
			...
			<div class="narrow_right fr">
				...
				<input type="hidden" name="poll_id" value="<?php echo $this->_tpl_vars["IN"]["id"];?>" />
						...
						search_url_submit.click(function(){
							...
							$.ajax({
								...
								{if(json == null || json.length == 0){
									...
									var price = $(document.createElement("span")).addClass("hide").attr("id","good_price").text("￥"+toDecimal2(json.goodsUnitPrice));
									item.append(img);
									item.append(name);
									item.append(intro);
									item.append(price);
