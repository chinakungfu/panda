...
$list_info = runFunc("getShareListByIdAndUser",array($this->_tpl_vars["IN"]["id"],$this->_tpl_vars["name"]));
...
<body>
	...
	<form id="list_submit_form" action="/publish/index.php" method="post" onSubmit="return formCheck()">
    	<div class="collection_top_bar oh">
            <div class="collection_title" style="color:#5e97ed;margin-bottom:10px;">
                <span>Edit a Collection!</span>
            </div>
        </div>
		<div class="narrow_box">
			<div class="narrow_left fl">
				...
				<div class="gray_line_box narrow_line_box">
						<table class="list_editor_table">
								...
								<td>
                                	<select name="categoryID" id="categoryID" style="color: #777777;width:160px;height:24px;line-height:24px; vertical-align:middle;font-size:13px;">
                                        ...
                                        <?php foreach($listCategory as $cate => $cateVal):?>
                                        	<option <?php if($cateVal['id'] == $list_info[0]["categoryID"]):?>selected="selected"<?php endif;?> value="<?php echo $cateVal['id'];?>"><?php echo $cateVal['name'];?></option>
                                        <?php endforeach;?>
                                    </select>
								</td>
							</tr>                         
							<tr>
								...
								<td>
									<input value="<?php echo $list_info[0]["title"];?>" class="list_editor_input big_input" type="text" id="list_name" maxlength="80" name="list_name"/>
									<br/>
							...
							<tr>
								...
								<td>
									<textarea onkeyup="checkWordLen(this);" name="description" class="list_editor_input list_editor_textarea"><?php echo $list_info[0]["description"];?></textarea>
									<br/>
							...
							<tr>
								<th>Privacy</>
								<td>
									<input <?php if($list_info[0]["privacy"]==0){echo 'checked="checked"';}?> type="radio" checked="checked" id="public" name="privacy" value="0"/> <label style="color:#4e4c4c;font-size:12px;" for="public">Public</label>&nbsp;&nbsp;&nbsp;&nbsp;
									<input <?php if($list_info[0]["privacy"]==1){echo 'checked="checked"';}?> type="radio" id="private" name="privacy" value="1"/> <label style="color:#4e4c4c;font-size:12px;" for="private">Private</label>
								</td>
								<td>
							...
							<tr>
								...
								<td>
									...
									<input type="radio" <?php if($list_info[0]["publish"]==1){echo 'checked="checked"';}?> id="publish" name="published" value="1"/> <label style="color:#4e4c4c;font-size:12px;" for="publish">Publish</label>&nbsp;&nbsp;&nbsp;&nbsp;
									<input type="radio" <?php if($list_info[0]["publish"]==0){echo 'checked="checked"';}?> id="unpublish" name="published" value="0" /> <label style="color:#4e4c4c;font-size:12px;" for="unpublish">Unpublish</label>
								</td>
							</tr>
						</table>
				</div>
				...
				<div class="list_item_contain gray_line_box narrow_line_box">
					<?php $items = runFunc("getMemberShareListItem",array($list_info[0]["id"]));?>
					<?php foreach($items as $item):?>
					<div class="added_items">
						<div class="added_item_img fl">
							<img src="<?php echo $item["goodsImgURL"];?>" alt=""/>
						</div>
						<div class="added_info_desc_box fl">
							<input class="added_item_info fl" type="text" name="added_item_title[<?php echo $item["goodsid"];?>]" value="<?php echo $item["list_item_title"];?>" maxlength="80">
							<textarea class="added_item_desc" name="added_item_desc[<?php echo $item["goodsid"];?>]"><?php if(trim($item["list_item_desc"])==""){echo "Maximum of 300 characters";}else{ echo $item["list_item_desc"];}?></textarea>
						</div>
						<div class="del_added_item fr">X</div>
						<input type="hidden" name="add_item_id[<?php echo $item["goodsid"];?>]" value="<?php echo $item["goodsid"];?>">
					</div>
					<?php endforeach;?>
				</div>
			</div>
