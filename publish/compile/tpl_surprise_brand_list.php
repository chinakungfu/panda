	...
	<div class="full_width_box oh">
		...
		<?php $category = runFunc("getAllShopsBrandCategoryById",array($this->_tpl_vars["IN"]["category_id"]));?>
			...
			<div id="catcont">
				...
				<a href="<?php echo runFunc('encrypt_url',array('action=surprise&method=brand_list'));?>"> Show All Shops</a>
				</div>
				<div class="catpost">
		...
		<?php if(count($items)<1){continue;}?>
			<div id="catcont">
				...
				<a href="<?php echo runFunc('encrypt_url',array('action=surprise&method=brand_list&category_id='.$category["id"]));?>">Show More</a>
				</div>
				<div class="catpost">
		...
		<div id="brand_list_right_select" class="fr">
			<div id="sidebarnav">
				<div class="menu-c-container">
					...
					<ul class="brand_list_menu" id="menu-c">
						...
						<li class="menu-item menu-item-type-custom menu-item-object-custom current-menu-item current_page_item menu-item-home menu-item-7535" id="menu-item-7535"><a style="color:#5E97ED;" href="<?php echo runFunc('encrypt_url',array('action=surprise&method=brand_list'));?>">All Shops</a></li>
					<?php foreach($categories as $category):?>
						<li class="menu-item menu-item-type-taxonomy menu-item-object-category menu-item-7528" id="menu-item-7528"><a href="<?php echo runFunc('encrypt_url',array('action=surprise&method=brand_list&category_id='.$category["id"]));?>"><?php echo $category["name"];?></a></li>
					<?php endforeach;?>
					</ul>
				</div>
			</div>
			...
			<?php $tags = runFunc("getAllShopBrandTags");
				foreach($tags as $tag):
			?>
			<a style="font-size: 8pt;" href="<?php echo runFunc('encrypt_url',array('action=surprise&method=brand_list&tag_id='.$tag["id"]));?>"><?php echo $tag["name"];?></a>	
			<?php endforeach;?>
			</div>
	</div>
</div>
