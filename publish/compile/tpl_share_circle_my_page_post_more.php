...
$posts = runFunc("getCircleMyJoinPost",array($this->_tpl_vars["name"],$this->_tpl_vars["IN"]["page"],5));
$html = "";
...
foreach ($posts as $key=>$post){
	...
	if(count($imgs)>0){
		...
		$html .= 'From: <a href="'.runFunc('encrypt_url',array('action=share&method=circlePage&id='.$post["circle_id"])).'">'.$post["name"].'</a>';
