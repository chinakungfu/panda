	...
	if( $ack == "SUCCESS" || $ack == "SUCESSWITHWARNING")
	{
		...
		*/
		...
		if($resConfirmArray["PAYMENTINFO_0_PAYMENTSTATUS"]=="Completed"){
			updateRechargePhoneSuccess($resArray["PAYMENTREQUEST_0_INVNUM"]);
			header("Location: /publish/index.php".runFunc('encrypt_url',array('action=shop&method=send_phonePayment_success_mail&orderID='.$resArray["PAYMENTREQUEST_0_INVNUM"])));
		}
		else{
...
function getPhoneOrderbyPaypal($order_id){
	...
	$db_config = $GLOBALS['currentApp']['dbconfig'];
		...
		$sql = "select * from cms_publish_phone_order where id = '{$order_id}'";
