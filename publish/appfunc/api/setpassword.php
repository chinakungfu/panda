...
function start_edit(&$var){
	if($var['newpwd']!=$var['password']){
		smsg("<font color='#ff0000'>修改密码出错，两次输入密码不一样...</font>",$_GET['referer']);
		return false;
	}
	$var['password']=md5($var['password']);
	$var['oldpwd']=md5($var['oldpwd']);
	...
	if($username=readsession()){
		if(!$user=unserialize($_SESSION['qy_member_info'])){
			$sql="select staffId, password from cms_member_staff where staffNo='$username' limit 1";
			$user=TStaticQuery::queryList($GLOBALS['currentApp']['dbaccess'], $sql);
			$user=$user[0];
		}
	}
	
	if($user['staffId']!=$_GET['contentId']){
		smsg("<font color='#ff0000'>修改密码出错，你可能已经退出登录，请重新登录再试...</font>",'/login.html');
		return false;
	}
	
	if($user['password']!=$var['oldpwd']){
		smsg("<font color='#ff0000'>修改密码出错，原密码不正确...</font>",'/login.html');
		return false;
	}
...
function end_edit($var){
	smsg("<font color='#ff0000'>修改密码成功...</font>",'/login.html');
}
