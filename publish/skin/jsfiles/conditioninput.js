document.write("<scr"+"ipt language=javascript  src=DateTimeCalendar.js></scr"+"ipt>");
function sqlcondition(tableId)
{
	...
	//预算符
	var operationValue=new Array(new Array("like","包含"),new Array("=","等于"),new Array(">=","大于等于"),new Array(">","大于"),
	new Array("<=","小于等于"),new Array("<","小于"),new Array("<>","不等于"),
...
//		leftBracketHeader.innerHTML="左括号";
//		leftBracketHeader.style.borderColor="#9FD6FF";		
	  ...
	  var operationHeader=header.insertCell(header.cells.length);
		...
		fieldContextHeader.innerHTML="内容";
...
//		rightBracketHeader.innerHTML="右括号";
...
//		logicSelect.style.borderColor="#9FD6FF";		
		  ...
		  for (j=0;j<dictList.length;j++)
		  {
		  	...
		  	{		  		
		  		var ooption=document.createElement("Option");
					...
					ooption.innerHTML=dictList[j][2];
		...
		else
		{
			...
			{				
				inputBox.onkeypress=function()
				{
					...
					{
						event.returnValue = false;
        				listResource.document.all("select").click();
					}
					else
					{
			...
			else
			{
				inputBox.onkeypress=function()
				{
					...
					{
						event.returnValue = false;
        				listInfo.document.all("select").click();
					}
				}
			}
			//在焦点失去事件中，设置值数组
	...
	this.newFieldSelect=function(fieldList,obj)
	{
		...
		fieldSelect.onchange=function()
		{
			...
			table.rows[rownum+1].cells[2].removeChild(table.rows[rownum+1].cells[2].children[0]);
			table.rows[rownum+1].cells[2].appendChild(obj.newContentInput(obj.dictList,fieldList,rownum));			
			var elementwidth=table.rows[rownum+1].cells[2].clientWidth -4;			
			table.rows[rownum+1].cells[2].children[0].style.cssText="width:"+elementwidth
		}
		return fieldSelect;
	}
	...
	{				
		...
		{					
			...
			ooption.innerHTML =leftBracketValue[i][0];
	...
	this.newRightSelect=function()
	{
		...
		for (i=0;i<rightBracketValue.length;i++)
		{
			...
			ooption.innerHTML=rightBracketValue[i][0];
...
//创建运算符
	this.newOperationSelect=function()
	{
		...
		for (i=0;i<operationValue.length;i++)
		{
			...
			ooption.innerHTML=operationValue[i][1];
...
//创建逻辑运算符
	this.newLogicSelect=function(obj)
	{
		...
		for (i=0;i<logicValue.length;i++)
		{
			...
			ooption.innerHTML=logicValue[i][1];
