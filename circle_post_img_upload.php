...
$user_id = $_GET['id'];
$filename = "circle_post_".time();
$ext = @$pathinfo['extension'];
if(!is_dir("circle_post_img/".$user_id)){
	mkdir("circle_post_img/".$user_id);
}

$uploadDirectory = "circle_post_img/".$user_id."/";
...
$uploadDirectory . $filename . '.' . $ext;
	make_thumb($uploadDirectory . $filename . '.' . $ext,$uploadDirectory . $filename,500);
	make_thumb($uploadDirectory . $filename . '.' . $ext,$uploadDirectory . "thumb_".$filename,120);
